...
- How many customers were created in the database?
    R:/ In the database are created 5 customers Alice, Bob, Jimmy, Freddy and Michael
- Where is the *findAll* method implemented?
    R:/ The findAll method is implemented in the CrudRepository class that is the one that extends CustomerRepository and ProductRepository
- Suppose you have more than 1000 products in your database. How would you implement a method for supporting pagination and return pages of 50 products to your frontend?
    R:/ I implement a new method in the ProductRepository that receive a Pageable object, in this Pageable object I set a size of 50, now I can send that List to the Front
- How many products contain the "plus" 
    R:/ 4 products contains a Plus
- How many products are returned by the *findByDescriptionContaining* query? Why?
    R:/ The query return 2 products because we put a Pageable object that have a limit size of 2 "Pages" (2 products)
- Which are the collection names where the objects are stored? Where are those names assigned?
    R:/ The collections name are customer and product, I think that name are the model's names but in full lowercase
